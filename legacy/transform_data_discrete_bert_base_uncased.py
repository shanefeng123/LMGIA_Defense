...
nearest_neighbors = {}
nearest_neighbors_similarities = {}
for i in range(len(embeddings)):
    if i not in tokenizer.all_special_ids:
        ...
        nearest_neighbors_similarities[i] = cosine_similarities[sorted_indices[1:k + 1]].tolist()
...
for batch in dataloader:
    ...
    optimizer = BeamSearchOptimizer(model, batch_hidden_states, batch, device=device,
                                        neighbours=nearest_neighbors, tokenizer=tokenizer,
                                        embedding_layer=embedding_layer, beam_size=beam_size,
                                        iteration=10,
                                        alpha=0.7, beta=0.3)
    ...
    i += 1
#
with open(f"../data/bert-base-uncased/{num_of_samples}_transformed_{data}_beam_{beam_size}_data.json", "w") as f:
