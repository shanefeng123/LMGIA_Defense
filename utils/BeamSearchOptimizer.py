...
class BeamSearchOptimizer:
    def __init__(self, model, hidden_states, batch, device, neighbours, tokenizer, embedding_layer,
                 beam_size=1,
                 iteration=10, alpha=0.7,
                 beta=0.3):
        self.model = model
        ...
        for i in range(len(batch_tokens)):
            ...
            for token in tokens:
                if token in self.tokenizer.all_special_ids:
                    ...
                    # while True:
                    #     #TODO: dead loop
                    #     dummy_token = random.choice(self.neighbours[token])
                    #     doc = spacy_nlp(
                    #         self.tokenizer.decode(token).strip() + " " + self.tokenizer.decode(dummy_token).strip())
                    #     if str.lower(doc[0].lemma_) != str.lower(doc[1].lemma_):
                    #         break
                    # dummy_tokens.append(dummy_token)
    ...
    def get_min_max_mse(self):
        # TODO: The max_output_mse can be too large and becomre inf. Cap it to a threshold value
        batch_tokens = self.batch["input_ids"].tolist()
        dummy_tokens = []
        for i in range(len(batch_tokens)):
            tokens = batch_tokens[i]
        ...
        #             else:
        #                 dummy_tokens.append(random.choice(self.neighbours[token]))
        #     output_mse, embed_mse = self.get_mse([dummy_tokens])
        #     output_mses.append(output_mse)
        #     embed_mses.append(embed_mse)
        # max_output_mse = max(output_mses)
        # max_embed_mse = max(embed_mses)
        # min_output_mse = min(output_mses)
        # min_embed_mse = min(embed_mses)
    ...
    def get_mse(self, solution):
        ...
        with torch.no_grad():
            solution_embedding = self.embedding_layer(torch.tensor(solution).to(self.device))
            batch_embedding = self.embedding_layer(self.batch["input_ids"].to(self.device))
                    ...
                    for l in range(len(sequence)):
                        if self.valid_neighbours[l] is None:
                            ...
                            for neighbour in self.valid_neighbours[l]:
                                # doc = spacy_nlp(
                                #     self.tokenizer.decode(batch_tokens[0][l]).strip() + " " + self.tokenizer.decode(
                                #         neighbour).strip())
                                # if str.lower(doc[0].lemma_) == str.lower(doc[1].lemma_):
                                #     continue
