...
device = args.device
result_file = f"../results/{model_name}/{num_of_samples}_original_{data}_b_{batch_size}_result.txt"
...
train_sentences = sentences[:int(num_of_samples * 0.8)]
...
train_tokenized_sentences = tokenizer(train_sentences, return_tensors="pt", padding=True, truncation=True)
train_tokenized_sentences["labels"] = torch.LongTensor(train_labels)
test_tokenized_sentences = tokenizer(test_sentences, return_tensors="pt", padding=True, truncation=True)
test_tokenized_sentences["labels"] = torch.LongTensor(test_labels)
train_set = MyDataset(train_tokenized_sentences)
test_dataset = MyDataset(test_tokenized_sentences)
...
with open(result_file, "a") as file:
    file.write("Original model testing: \n")
